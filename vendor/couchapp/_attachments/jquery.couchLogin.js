        ...
        elem.delegate("form.signup", "submit", function() {
            ...
            $.couch.signup({name : name}, pass, {
                ...
                error : function(x,y,z){
                  if (x=409){
                    alert("Looks like someone has that name, try again");
                  }
...
//bootstrap compatible templates
    var templates = {
        adminParty : '<p><strong>Admin party, everyone is admin!</strong> Fix this in <a href="/_utils/index.html">Futon</a> before proceeding.</p>',
        loggedOut : '<a class="btn" href="#login">Login</a><a class="btn"',
        loginForm : '<form class="login">  <input placeholder="Name" type="text" name="name" value="" autocapitalize="off" autocorrect="off"><input type="password" placeholder="Password" name="password" value=""><input class="btn" type="submit" value="Login"></form>',
        signupForm : '<form class="signup"><input placeholder="Name" type="text" name="name" value="" autocapitalize="off" autocorrect="off"><input type="password" placeholder="Password" name="password" value=""><input class="btn" type="submit" value="Signup"><button class="btn" href="#login">Login</button></form>',
        loginButtons :''
        };
    ...
    function loggedIn(r) {
        var auth_db = encodeURIComponent(r.info.authentication_db)
        , uri_name = encodeURIComponent(r.userCtx.name)
        , span = $('<a class="btn" href="#logout">Logout</a>');
        $('a.name', span).text(r.userCtx.name); // you can get the user name here
        return span;
    }
